...
// 考古題資料（init() 載入後填入）
var examIndex = {};   // indicatorCode → [{year,grade,q_no,stem,options,answer,has_image,image_path}]
...
function renderExamQuestions(unit) {
  ...
  var items = qs.map(function(q) {
    ...
    if (!q.has_image && q.options && Object.keys(q.options).length) {
      optHtml = '<div class="exam-q-options">' +
        [1,2,3,4].filter(function(i){ return q.options[String(i)]; })
          .map(function(i){ return '<div class="exam-q-option">' + syms[i-1] + ' ' + q.options[String(i)] + '</div>'; })
          .join('') +
        '</div>';
    }
    var imgHtml = '';
    ...
    return '<div class="exam-q-item">' +
      ...
      (stem ? '<div class="exam-q-stem">' + stem + '</div>' : '') +
      optHtml + imgHtml +
    '</div>';
  }).join('');
