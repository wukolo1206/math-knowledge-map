...
const g3bConceptIds = new Set(
  concepts.filter((concept) => concept.unit_id.startsWith('G3B-')).map((concept) => concept.concept_id)
);

let updated = 0;

for (const unit of units) {
  if (unit.grade !== 3 || unit.semester !== 2) continue;

  const sourceUnitId = 'G3B-U' + unit.unit_number;
  ...
  unit.concept_relations = relations
    ...
    .filter((relation) => {
      ...
      return Boolean(from && to) && (g3bConceptIds.has(relation.from_id) || g3bConceptIds.has(relation.to_id));
    })
    .map((relation) => {
      const from = conceptById.get(relation.from_id);
      const to = conceptById.get(relation.to_id);
...
console.log('Synced G3B concepts for ' + updated + ' units');
