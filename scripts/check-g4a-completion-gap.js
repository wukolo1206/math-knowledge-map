...
for (const [unitNumber, expectedCount] of expectedCoreCounts.entries()) {
  const unit = units.find((item) => item.grade === 4 && item.semester === 1 && item.unit_number === unitNumber);
  if (!unit) {
    throw new Error('找不到四上第 ' + unitNumber + ' 單元');
  }
...
console.log('G4A completion gap check passed');
